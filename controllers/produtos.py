...
from schemas.produto.produto_schema import CriarProduto, ProdutoResposta, AtualizarProduto
...
def pegar_produto_por_id(id: int ,session: Session = Depends(pegar_sessao)):
    return produto_service.pegar_produto_por_id(id, session)


@produtos_router.put("/{id}", response_model=ProdutoResposta)
def atualizar_produto(dados: AtualizarProduto,session: Session = Depends(pegar_sessao)):
